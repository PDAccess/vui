          ...
          <b-col cols="12" sm="6">
            <div class="module-nav">
              <b-form-input
                ...
                size="md"
                placeholder="Entry User info to search"
                type="text"
                v-model="tempFilter"
              ...
              >
                <optgroup label="Sort By">
                  ...
                  <option value="createddesc">
                    <strong>Created Time (DESC)</strong>
                  </option>
                </optgroup>
              </b-form-select>
            </div>
        ...
        >
          <template v-slot:cell(userName)="row">
            <div class="d-flex align-items-center">
              <div>
                <b-button
                  @click="userDetailPage(row.item.userId)"
          ...
          <template v-slot:cell(authStatus)="row">
            <b-badge v-if="row.item.authStatus" variant="primary">
              Success
            </b-badge>
            <b-badge v-else variant="danger"   v-b-tooltip.hover :title="row.item.Reason"> Failed </b-badge>
          </template>

          <template v-slot:cell(attemptTime)="row">
            <p-timing :start-date="row.item.attemptTime" />
          </template>
        </b-table>
        <infinite-loading
...
import { LogApi, SearchAuthRequest } from '@/generated/authws';
...
export default {
  ...
  data() {
    return {
      ...
      sessionsfields: [
        ...
        { key: "remoteAddr", sortable: true, label: "Ip" },
        { key: "remoteAgent", sortable: true, label: "User Agent" },
        { key: "authStatus", label: "" },
        { key: "loginType", label: "" },
        { key: "attemptTime", label: "" },
      ],
      currentPage: 0,
      perPage: 15,
  ...
  methods: {
    ...
    getOnlineUsers() {
      ...
      let searchAuthRequest = {}

      new LogApi(this.$authHttpConfig, undefined, this.$http).
        authLogs(searchAuthRequest).
        then((response) => {
