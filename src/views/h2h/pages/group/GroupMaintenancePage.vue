<template>
  <div class="animated fadeIn">
    <p-modal
      v-if="deleteButtonValue == 'User'"
      @confirm="deleteUser()"
      :message="modalMessage"
    ></p-modal>
    <p-modal
      v-if="deleteButtonValue == 'Maintenance'"
      @confirm="deleteMaintenance()"
      :message="modalMessageMaintenance"
    ></p-modal>
    <p-card>
      <p-error
        v-if="errorStatus"
      ...
      <ul v-else class="list-group list-group-flush">
        <li>
          <b-row>
            ...
            <b-button variant="success" @click.stop="$router.push({ path: '/groups/maintenance/' + $route.params.id + '/create' })"
                    ><em class="fa fa-plus mr-2"/> Create Maintenance</b-button
                  >
            </b-col>
          </b-row>
          <b-row class="p-2">
            ...
            <div v-else-if="maintenanceList.length === 0" style="margin: auto">
              <p-initial
                icon="plus-circle"
              ...
              <template v-slot:cell(action)="row">
                ...
                >
                  ...
                  <b-dropdown-item
                    ...
                    @click.stop="$router.push({ path: '/groups/maintenance/' + id + '/edit/' + row.item.uniqueId })"
                    ><em class="fa fa-edit"></em>Edit</b-dropdown-item
                  >
                  <!--
                  <b-dropdown-item
                    :title="infoUsersFunction(row, row.index)"
                    @click.stop="details(row.item)"
                    ><em class="fa fa-info"></em>İnfo</b-dropdown-item
                  >
                  -->
                </b-dropdown>
              </template>
  ...
  methods: {
    ...
    uniqueMaintenance(items) {
      ...
      items.forEach((element) => {
        ...
        items.forEach(item => {
          if (element.startDate == item.startDate && element.endDate == item.endDate && element.username != item.username) {
            this.infoUsers += ","
            this.infoUsers += item.username
          }
        });
        dates = {
