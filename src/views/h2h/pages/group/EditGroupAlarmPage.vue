          ...
          <b-row class="my-4">
            ...
            <b-col cols="8" class="d-flex justify-content-end">
              <b-button
                ...
                class="mr-2"
                @click="updateAlarm"
                >Update Alarm</b-button
              >
              <b-button :to="'/groups/alarms/' + id" variant="danger"
                >Back</b-button
              >
            </b-col>
          </b-row>
...
export default {
  ...
  methods: {
    ...
    updateAlarm() {
      ...
      this.$http
        .put("/api/v1/alarm/" + this.alarmid, data)
        // eslint-disable-next-line no-unused-vars
        .then((response) => {
          this.$bvToast.toast(`Edited Alarm`, {
            title: "Group Alarm Notification",
            variant: "success",
            autoHideDelay: 5000,
          });
          this.$router.push({
  ...
  created() {
    this.$http
      ...
      .then((response) => {
        ...
        this.$http
          .get("api/v1/alarm/" + this.alarmid)
          .then((response2) => {
            this.formdata.name = response2.data.name;
            this.formdata.message = response2.data.message;
            this.formdata.description = response2.data.description;
            this.formdata.regex = response2.data.regex;
            response2.data.users.forEach((userId) => {
              this.users.forEach((user) => {
                if (user.userId === userId) {
                  this.formdata.users.push(user);
                }
              });
            });
            this.loading = false;
