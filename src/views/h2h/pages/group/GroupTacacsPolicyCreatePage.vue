  ...
  methods: {
    ...
    createPolicy() {
      let data = {
        ...
        "userIds": this.users.map(user => user.userId),
        "regexes": this.regex,
        "policyType": this.type
      };

      this.$http
        .post("/api/v1/tacacs/policy/group/" + this.id, data)
        .then((response) => {
          this.$bvToast.toast(`Created Tacacs Policy`, {
