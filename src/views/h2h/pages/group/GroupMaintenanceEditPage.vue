<template>
  <b-row class="my-5" align-h="between">
    <b-col cols="12" sm="3" md="3" lg="3">
      <p>Explanation of Maintenance Menu.</p>
    </b-col>
    <b-col
      cols="12"
      v-if="screen == 'createMaintenance'"
      sm="9"
      class="select-menu"
    >
      <b-row>
        <b-col md="6">
          <b-form-group for="selectedUsers" horizontal>
            <label class="typo__label">Allowed Users in Maintenance</label>
            <p-select
              v-model="value"
      ...
      <b-row class="mb-4">
        <b-col class="d-flex justify-content-end">
          <b-button variant="success" @click.stop="addMaintenance"
            >Edit</b-button
          >
        </b-col>
      </b-row>
    </b-col>
    <b-col v-if="screen == 'editMaintenance'" sm="9" class="select-menu">
      <b-row>
        <b-col md="6">
          <b-form-group for="selectedUsers" horizontal>
            <div>
              <label class="typo__label"
                >Editable Allowed Users in Maintenance</label
              >
              <p-select
                v-model="value"
                deselect-label="Can't remove this value"
      ...
      <b-row class="mb-4">
        <b-col class="d-flex justify-content-end">
          <b-button variant="success" @click.stop="editMaintenance" class="mr-2"
            >Edit</b-button
          >
          <b-button variant="info" @click.stop="clearForm">Clear Form</b-button>
        </b-col>
      </b-row>
    </b-col>
    <hr />
  ...
  methods: {
    ...
    editMaintenance() {
      ...
      this.$http
        ...
        .then((response) => {
          this.$router.push({ path: "/groups/maintenance/" + this.id });
          this.$http
            .get("/api/v1/maintenance/" + this.$route.params.id)
            .then((response2) => {
              this.items = response2.data;
    ...
    firstEditButton(items) {
      ...
      this.dateOldRange = {
        start: new Date(items[0].startDate),
        end: new Date(items[0].endDate),
      };
  ...
  created() {
    this.$http
      ...
      .then((response) => {
        ...
        this.$http
          .get("/api/v1/maintenance/unique/" + this.maintenanceId)
          .then((response) => {
            this.value = [];
            this.items = response.data;
            this.firstEditButton(this.items);
