      ...
      >
        ...
        <template v-slot:cell(username)="row">
          <span class="text-info">{{
            row.item.username + "@" + serviceInfo.name
          }}</span>
        </template>

        <template v-slot:cell(user_id)="row">
          <span class="text-info">{{ row.item.userId }}</span>
        </template>

        <template v-slot:cell(e_username)="row">
          <span class="text-info">{{ row.item.eUsername }}</span>
        </template>

        <template v-slot:cell(e_user_id)="row">
          <span class="text-info">{{ row.item.eUserId }}</span>
        </template>

        <template v-slot:cell(ip_address)="row">
          <span class="text-info">{{ row.item.clientIp }}</span>
        </template>
        ...
        <template v-slot:cell(login_time)="row">
          <span
            v-if="row.item.startTime"
            class="text-info"
            :title="$moment(row.item.startTime).calendar()"
            >{{
              $moment(row.item.startTime).format("DD/MM/YYYY-HH:mm:ss")
            }}</span
          >
        </template>
        ...
        <template v-slot:cell(logout_time)="row">
          <span
            v-if="row.item.endTime"
            class="text-info"
            :title="$moment(row.item.endTime).calendar()"
            >{{ $moment(row.item.endTime).format("DD/MM/YYYY-HH:mm:ss") }}</span
          >
        ...
        <template v-slot:cell(action)="row">
          <b-button
            :to="
              '/service/agentShellActivities/session/' + id + '/' + row.item.id
            "
            variant="primary"
            class="fa fa-info"
          ></b-button>
...
export default {
  ...
  computed: {
    ...
    isLoading() {
      return this.externalCommandLoading || this.serviceInfoLoading;
    },
  },
  methods: {
    getServiceInfo() {
