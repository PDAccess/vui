...
export default {
  ...
  methods: {
    ...
    infiniteHandlerAuth($state) {
      ...
      this.$http
        .post("/api/v1/auths/service/" + this.id, dataPolicy)
        .then((response) => {
          if (response.data.length) {
      ...
      this.$http
        ...
        .then((response) => {
          ...
          response.data.logs.forEach((f) => {
            if (this.authenticationId && f.id == this.authenticationId) {
              f._rowVariant = 'primary'
            }
            this.authsessions.push(f);
          });
  ...
  created() {
    ...
    if (this.$route.hash) {
      this.authenticationId = this.$route.hash.substring(1)
    }
