<template>
  <p-card>
    ...
    <template v-else>
        ...
        <b-col>
          <b-row>
            ...
            <b-col cols="1" sm="1" class="d-flex justify-content-center">
              <download-excel
                :fetch="exportExcel"
                :name="`Tacacs-Authorization-Logs-${id}.xls`"
                :fields="excelField"
      ...
      <p-initial
        v-else-if="
          tacacsAuthorizationLogs === null ||
          tacacsAuthorizationLogs.length === 0
        "
      ...
      <template v-else>
        <b-table
          :fields="tacacsAuthorizationLogFields"
          :items="tacacsAuthorizationLogs"
        ...
        >
          ...
          <template v-slot:cell(args)="row">
            <span class="text-info">{{ row.item.args }}</span>
          </template>

          <template v-slot:cell(privileged_level)="row">
            <span class="text-info">{{ row.item.privilegedLevel }}</span>
          </template>

          <template v-slot:cell(reason)="row">
            <span class="text-info">{{
              row.item.status ? "Successfully Authorization" : row.item.reason
            }}</span>
          </template>
        ...
        <infinite-loading
          @infinite="updateTacacsAuthorizationLogs"
...
export default {
  ...
  data() {
    return {
      excelField: {
        ...
        Args: "args",
        "Privileged Level": "privilegedLevel",
        Reason: "reason",
        Status: "status",
      },
      excelLoading: false,
      ...
      errorResponse: "",
      loading: true,
      tacacsAuthorizationLogs: [],
      tacacsAuthorizationLogFields: [
        ...
        { key: "args", label: "Args" },
        { key: "privileged_level", label: "Privileged Level" },
  ...
  watch: {
    dateRangeData: function (value) {
      this.getTacacsAuthorizationLogs();
    },
    liveMode(value) {
      if (value) {
        this.lastTacacsAuthorizationLogs();
        this.liveAction = setInterval(this.lastTacacsAuthorizationLogs, 5000);
      } else {
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      let logs = await this.$http
        .post(`/api/v1/tacacs/logs/authorization/${this.id}?${queryParams}`)
        .then((response) => {
      ...
      logs.forEach((f) => {
        excelData.push({
          ...
          args: f.args,
          privilegedLevel: f.privilegedLevel,
          reason: (function () {
            if (f.reason) {
              return f.reason;
            }
    ...
    updateTacacsAuthorizationLogs($state) {
        ...
        .then((response) => {
          if (response.data.data.length) {
            ...
            response.data.data.forEach((element) => {
              this.tacacsAuthorizationLogs.push(element);
            });
            $state.loaded();
          } else {
            $state.complete();
    ...
    getTacacsAuthorizationLogs() {
      ...
      this.$http
        ...
        .then((response) => {
          this.offset += this.limit;
          this.tacacsAuthorizationLogs = response.data.data;
          this.loading = false;
        })
        .catch((error) => {
          console.log(error);
