  ...
  <div class="wrapper">
    <div class="animated fadeIn">
      ...
      <div
        ...
        v-else-if="serviceAccountUsersData.length === 0"
      >
        <empty-data textmessage="No Data!" />
      </div>
      <div v-else>
        <b-table :items="serviceAccountUsersData" show-empty hover>
          ...
          <template v-slot:cell(isManage)="row">
            <b-badge :variant="row.item.isManage ? 'success' : 'danger'">{{
              row.item.isManage ? "enable" : "disable"
            }}</b-badge>
          </template>
        </b-table>
      </div>
...
export default {
  name: "ServiceAccountUsersReports",
  components: {
    DownloadExcel,
  },
  data() {
  ...
  methods: {
    ...
    getData() {
      ...
      this.serviceAccountUsersData = datas;
      this.loading = false;
    },
  },
  created() {
