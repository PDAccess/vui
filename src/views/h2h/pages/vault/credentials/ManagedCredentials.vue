  ...
  <div class="animated fadeIn">
    <p-card :cardBody="true">
      ...
      ">
        ...
        -->
              <!-- <b-button
                :disabled="!row.item.permissionsSet.includes('CAN_CONNECT') || row.item.credentialEntity.connectionUser == null"
                variant="primary">
      ...
      goToConnectionPage(
        row.item.serviceId,
        row.item.credentialId)" size=" sm" variant="outline-success">
...
export default {
  ...
  computed: {
    ...
    breakPasskey() {
      let keyValuePasskey = this.breakCredential.keyValue.filter(
        (keyValue) => keyValue.key.includes('passkey')
      )
      if (keyValuePasskey.length == 0) {
        return null
      }

      return keyValuePasskey[0]
    },
    breakPassword() {
      let keyValuePassword = this.breakCredential.keyValue.filter(
        (keyValue) => keyValue.key.includes('password')
      )
      if (keyValuePassword.length == 0) {
        return null
      }

      return keyValuePassword[0]
    }
  },
  methods: {
    ...
    goToConnectionPage(serviceId, credentialId) {
      window.open(
        `https://${this.settings.default_system_host_name}/term/console/?token=${this.userToken}&serviceId=${serviceId}&type=ssh&credentialId=${credentialId}`,
        "_blank",
    ...
    checkOutBreak(item) {
      new BreakTheGlassApi(this.$cmanagerHttpConfig, undefined, this.$http)
        .breakCredential(credentialId, true)
        .then((response) => {
